...
- Records a local, prompt-free provider activity receipt—reused/opened tab, inserted draft/sent, or failure reason—so browser routing behaviour is diagnosable from the popup.
...
- The active-page palette includes editable summary, claims, study-notes, and critique recipes for quick analysis without locking you into a fixed prompt.
- Its context mode is explicit: **Prompt only**, **Selection**, or **Page excerpt**. It defaults to Prompt only when opened on a provider page so incidental selected UI text never pollutes a normal chat message.
