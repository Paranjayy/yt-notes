...
async function refreshStatus() {
  ...
  try {
    ...
    const kind = platform.startsWith('spotify') ? 'Spotify episode' : platform === 'x' ? 'X post' : platform === 'reddit' ? 'Reddit page' : platform === 'rosint' ? 'Rosint archive' : platform === 'deepwiki' ? 'DeepWiki page' : 'YouTube video';
    if (!response?.videoId && !response?.episodeId) {
      ...
      metaEl.textContent = platform.startsWith('spotify')
        ...
        : platform === 'x'
          ...
          : platform === 'rosint'
            ? 'Open a Rosint profile (rosint.dev/?u=<name>) to capture it.'
            : platform === 'deepwiki'
              ? 'Open a DeepWiki repo or page to capture it.'
      ...
      metaEl.textContent = response.transcriptAvailable ? 'Content is ready locally.' : (platform.startsWith('spotify') ? 'Transcript not synced yet — it auto-syncs, or press Sync in the page widget.' : platform === 'x' ? 'Posts not captured yet — they auto-read, or press Capture in the page widget.' : platform === 'reddit' ? 'Not captured yet — it auto-reads, or press Capture in the page widget.' : platform === 'rosint' ? 'Not captured yet — it auto-reads, or press Capture all in the page widget.' : platform === 'deepwiki' ? 'Not captured yet — it auto-reads, or press Capture in the page widget.' : 'Transcript not saved yet — Sync it in the page widget.');
      captureLabel.disabled = false;
      captureLabel.checked = true;
      setCaptureLabel(platform.startsWith('spotify') ? 'Use saved Spotify capture' : platform === 'x' ? 'Use saved X capture' : platform === 'reddit' ? 'Use saved Reddit capture' : platform === 'rosint' ? 'Use saved Rosint capture' : platform === 'deepwiki' ? 'Use saved DeepWiki capture' : 'Use saved YouTube capture');
    }
  } catch (error) {
  ...
  try {
    ...
    if (document.getElementById('preferCapture').checked) {
      ...
      if (capture?.ok && capture.markdown) {
        ...
        const kind = capture.platform === 'x' ? 'X' : capture.platform === 'reddit' ? 'Reddit' : capture.platform === 'rosint' ? 'Rosint' : capture.platform === 'deepwiki' ? 'DeepWiki' : capture.platform?.startsWith?.('spotify') ? 'Spotify' : 'YouTube';
